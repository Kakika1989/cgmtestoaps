...
function init (env, ctx) {
  ...
  function levelToDisplay (level) {
    ...
    return key && ctx.language.translate(levels.level2Display[key]) || ctx.language.translate('Unknown');
  }
  ...
  function getAlarm (level, group) {
    ...
    if (!alarm) {
      var display = group === 'default' ? levelToDisplay(level) : group + ':' + level;
  ...
  function notifyToView (notify) {
    return {
      level: levelToDisplay(notify.level)
      , title: notify.title
  ...
  function snoozeToView (snooze) {
    return {
      level: levelToDisplay(snooze.level)
      , title: snooze.title
